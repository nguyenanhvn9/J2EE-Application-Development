...
import org.example.booking.service.AddressService;
...
public class CartController {
    ...
    public String applyVoucher(
        ...
        // Apply voucher without validity check
        Voucher voucher = voucherService.findByCode(voucherCode);
        if (voucher == null) {
            redirectAttributes.addFlashAttribute("errorMessage", "Voucher not found.");
            return "redirect:/cart";
        }

        cart.setVoucher(voucher);
        cart.setDiscount(voucher.getDiscountAmount());

        shoppingCartService.save(cart);

        redirectAttributes.addFlashAttribute("successMessage", "Voucher applied successfully.");
        return "redirect:/cart";
    }
