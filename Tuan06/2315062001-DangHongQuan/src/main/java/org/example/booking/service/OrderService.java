...
public class OrderService {
    ...
    public List<Order> findCompletedOrdersByUser(Long userId) {
    ...
    public List<Order> findDeliveredOrdersByUser(Long userId) {
        return orderRepository.findByUserIdAndStatus(userId, "DELIVERED");
    }
    public void applyVoucherToOrder(Order order, String code) {
        Optional<Voucher> optionalVoucher = voucherRepository.findByCode(code);
        if (optionalVoucher.isPresent() && optionalVoucher.get().isValid()) {
            order.applyVoucher(optionalVoucher.get());
            orderRepository.save(order);
        } else {
